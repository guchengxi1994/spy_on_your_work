import 'package:flutter/material.dart';
import 'package:spy_on_your_work/src/app/application/application_state.dart';
import 'package:spy_on_your_work/src/app/application/components/cached_app_icon.dart';
import 'package:spy_on_your_work/src/isar/apps.dart';
import 'package:spy_on_your_work/src/app/app_catalog/app_catalog_notifier_simple.dart';

/// 优化的分类卡片组件，支持响应式布局
class OptimizedCategoryCard extends StatefulWidget {
  final IAppTypes type;
  final List<ApplicationUsage> apps;
  final Function(ApplicationUsage app, IAppTypes targetType) onAppMoved;
  final bool isCompactMode; // 是否为紧凑模式

  const OptimizedCategoryCard({
    super.key,
    required this.type,
    required this.apps,
    required this.onAppMoved,
    this.isCompactMode = false,
  });

  @override
  State<OptimizedCategoryCard> createState() => _OptimizedCategoryCardState();
}

class _OptimizedCategoryCardState extends State<OptimizedCategoryCard> {
  bool _isDragOver = false;

  @override
  Widget build(BuildContext context) {
    final displayName = categoryDisplayNames[widget.type] ?? '未知';
    final icon = categoryIcons[widget.type] ?? Icons.apps;
    final color = categoryColors[widget.type] ?? Colors.grey;

    return DragTarget<ApplicationUsage>(
      // onWillAccept: (data) => data != null,
      onAcceptWithDetails: (app) {
        widget.onAppMoved(app.data, widget.type);
        setState(() {
          _isDragOver = false;
        });
      },
      onMove: (details) {
        if (!_isDragOver) {
          setState(() {
            _isDragOver = true;
          });
        }
      },
      onLeave: (data) {
        setState(() {
          _isDragOver = false;
        });
      },
      builder: (context, candidateData, rejectedData) {
        return AnimatedContainer(
          duration: const Duration(milliseconds: 200),
          margin: const EdgeInsets.all(4),
          padding: EdgeInsets.all(widget.isCompactMode ? 8 : 12),
          width: double.infinity, // 占满容器宽度
          height: double.infinity, // 占满容器高度
          decoration: BoxDecoration(
            color: _isDragOver ? color.withValues(alpha: 0.1) : Colors.white,
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: _isDragOver ? color : const Color(0xFFE5E7EB),
              width: _isDragOver ? 2 : 1,
            ),
            boxShadow: [
              BoxShadow(
                color: Colors.black.withValues(alpha: _isDragOver ? 0.1 : 0.05),
                blurRadius: _isDragOver ? 8 : 4,
                offset: const Offset(0, 2),
              ),
            ],
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // 分类标题
              Row(
                children: [
                  Container(
                    padding: EdgeInsets.all(widget.isCompactMode ? 6 : 8),
                    decoration: BoxDecoration(
                      color: color.withValues(alpha: 0.1),
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Icon(
                      icon,
                      color: color,
                      size: widget.isCompactMode ? 16 : 20,
                    ),
                  ),
                  SizedBox(width: widget.isCompactMode ? 8 : 12),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          displayName,
                          style: TextStyle(
                            color: const Color(0xFF1F2937),
                            fontSize: widget.isCompactMode ? 14 : 16,
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                        Text(
                          '${widget.apps.length} 个应用',
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: widget.isCompactMode ? 10 : 12,
                          ),
                        ),
                      ],
                    ),
                  ),
                ],
              ),
              if (!widget.isCompactMode) ...[
                SizedBox(height: widget.isCompactMode ? 8 : 12),
                // 应用列表（仅在非紧凑模式下显示）
                Expanded(child: _buildAppsList()),
              ],
            ],
          ),
        );
      },
    );
  }

  Widget _buildAppsList() {
    if (widget.apps.isEmpty) {
      return Container(
        decoration: BoxDecoration(
          color: const Color(0xFFF8FAFC),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
            color: const Color(0xFFE5E7EB),
            style: BorderStyle.solid,
          ),
        ),
        child: Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.drag_indicator, color: Colors.grey[400], size: 20),
              const SizedBox(height: 4),
              Text(
                '拖拽应用到此处',
                style: TextStyle(color: Colors.grey[500], fontSize: 10),
              ),
            ],
          ),
        ),
      );
    }

    // 限制显示的应用数量，避免过度拥挤
    final displayApps = widget.apps.take(6).toList();
    final hasMore = widget.apps.length > 6;

    return SingleChildScrollView(
      child: Wrap(
        spacing: 4,
        runSpacing: 4,
        children: [
          ...displayApps.map((app) => _buildAppItem(app)),
          if (hasMore)
            Container(
              width: 32,
              height: 32,
              decoration: BoxDecoration(
                color: Colors.grey[200],
                borderRadius: BorderRadius.circular(6),
                border: Border.all(color: Colors.grey[300]!),
              ),
              child: Center(
                child: Text(
                  '+${widget.apps.length - 6}',
                  style: TextStyle(
                    color: Colors.grey[600],
                    fontSize: 10,
                    fontWeight: FontWeight.w500,
                  ),
                ),
              ),
            ),
        ],
      ),
    );
  }

  Widget _buildAppItem(ApplicationUsage app) {
    return Draggable<ApplicationUsage>(
      data: app,
      feedback: Material(
        elevation: 8,
        borderRadius: BorderRadius.circular(6),
        child: Container(
          width: 40,
          height: 40,
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(6),
            border: Border.all(color: Colors.grey[300]!),
          ),
          child: CachedAppIcon(iconData: app.icon, size: 24),
        ),
      ),
      childWhenDragging: Container(
        width: 32,
        height: 32,
        decoration: BoxDecoration(
          color: Colors.grey[200],
          borderRadius: BorderRadius.circular(6),
          border: Border.all(color: Colors.grey[300]!),
        ),
        child: Icon(Icons.apps, color: Colors.grey[400], size: 16),
      ),
      child: GestureDetector(
        onLongPress: () {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('长按拖拽 ${app.name} 到其他分类'),
              duration: const Duration(seconds: 1),
            ),
          );
        },
        child: Container(
          width: 32,
          height: 32,
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(6),
            border: Border.all(color: Colors.grey[300]!),
          ),
          child: CachedAppIcon(iconData: app.icon, size: 20),
        ),
      ),
    );
  }
}

/// 全屏拖拽源区域（用于未分类应用）
class DragSourceArea extends StatelessWidget {
  final List<ApplicationUsage> apps;
  final String title;
  final IconData icon;
  final Color color;

  const DragSourceArea({
    super.key,
    required this.apps,
    required this.title,
    required this.icon,
    required this.color,
  });

  @override
  Widget build(BuildContext context) {
    if (apps.isEmpty) {
      return const SizedBox.shrink();
    }

    return Container(
      margin: const EdgeInsets.all(8),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: const Color(0xFFE5E7EB)),
        boxShadow: [
          BoxShadow(
            color: Colors.black.withValues(alpha: 0.05),
            blurRadius: 4,
            offset: const Offset(0, 2),
          ),
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
                  color: color.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(6),
                ),
                child: Icon(icon, color: color, size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(
                      title,
                      style: const TextStyle(
                        color: Color(0xFF1F2937),
                        fontSize: 16,
                        fontWeight: FontWeight.w600,
                      ),
                    ),
                    Text(
                      '${apps.length} 个应用',
                      style: TextStyle(color: Colors.grey[600], fontSize: 12),
                    ),
                  ],
                ),
              ),
            ],
          ),
          const SizedBox(height: 12),
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: apps.map((app) => _buildDraggableApp(app)).toList(),
          ),
        ],
      ),
    );
  }

  Widget _buildDraggableApp(ApplicationUsage app) {
    return Draggable<ApplicationUsage>(
      data: app,
      feedback: Material(
        elevation: 8,
        borderRadius: BorderRadius.circular(8),
        child: Container(
          width: 48,
          height: 48,
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: Colors.grey[300]!),
          ),
          child: CachedAppIcon(iconData: app.icon, size: 32),
        ),
      ),
      childWhenDragging: Container(
        width: 40,
        height: 40,
        decoration: BoxDecoration(
          color: Colors.grey[200],
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: Colors.grey[300]!),
        ),
        child: Icon(Icons.apps, color: Colors.grey[400], size: 20),
      ),
      child: Container(
        width: 40,
        height: 40,
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: Colors.grey[300]!),
        ),
        child: CachedAppIcon(iconData: app.icon, size: 28),
      ),
    );
  }
}
